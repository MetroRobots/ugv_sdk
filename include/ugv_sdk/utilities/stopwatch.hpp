...
struct timer
{
  ...
  bool done() const
  {
    return done(Clock::now());
  }
  bool done(const time_point now) const
  {
    return now >= expiry;
  }
  ...
  duration remaining(const time_point now) const
  {
    return expiry - now;
  }
...
struct Timer
{
  ...
  {
    int64_t duration =
        period_ms - std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now() - tic_point).count();
    if (duration > 0)
  ...
  {
    int64_t duration =
        period_us - std::chrono::duration_cast<std::chrono::microseconds>(Clock::now() - tic_point).count();
    if (duration > 0)
