...
public:
  AgilexBase()
  {
    static_assert(std::is_base_of<ParserInterface<ProtocolVersion::AGX_V1>, ParserType>::value ||
                      std::is_base_of<ParserInterface<ProtocolVersion::AGX_V2>, ParserType>::value,
  ...
  {
    ConnectPort(can_name, std::bind(&AgilexBase<ParserType>::ParseCANFrame, this, std::placeholders::_1));
  }
  ...
  void EnableCommandedMode()
  {
    ...
    can_frame frame;
    if (parser_.EncodeMessage(&msg, &frame))
      can_->SendFrame(frame);
  }

  // must be called at a frequency >= 50Hz
  void SendMotionCommand(double linear_vel, double angular_vel, double lateral_vel, double steering_angle)
  {
    if (can_connected_)
    {
      ...
      if (parser_.GetParserProtocolVersion() == ProtocolVersion::AGX_V1)
      {
        ...
        msg.body.v1_motion_command_msg.angular =
            std::abs(angular_vel) > std::abs(steering_angle) ? angular_vel : steering_angle;
        msg.body.v1_motion_command_msg.lateral = lateral_vel;
      }
      else if (parser_.GetParserProtocolVersion() == ProtocolVersion::AGX_V2)
      {
  ...
  void SendLightCommand(LightMode front_mode, uint8_t front_custom_value, LightMode rear_mode,
...
protected:
  ...
  void UpdateActuatorState(const AgxMessage& status_msg)
  {
    ...
    case AgxMsgActuatorHSState:
    {
      ...
      actuator_state_msgs_.actuator_hs_state[status_msg.body.actuator_hs_state_msg.motor_id] =
    ...
    case AgxMsgActuatorLSState:
    {
      ...
      actuator_state_msgs_.time_stamp = AgxMsgRefClock::now();
      actuator_state_msgs_.actuator_ls_state[status_msg.body.actuator_ls_state_msg.motor_id] =
    ...
    case AgxMsgActuatorStateV1:
    {
      ...
      actuator_state_msgs_.time_stamp = AgxMsgRefClock::now();
      actuator_state_msgs_.actuator_state[status_msg.body.v1_actuator_state_msg.motor_id] =
