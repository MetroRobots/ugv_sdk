...
public:
  ...
  {
    AgilexBase::ConnectPort(dev_name, std::bind(&RangerBase::ParseCANFrame, this, std::placeholders::_1));
  }

  // robot control
  void SetMotionCommand(double linear_vel, double steer_angle, double lateral_vel = 0.0,
                        double angular_vel = 0.0) override
  {
    AgilexBase<ProtocolV2Parser>::SendMotionCommand(linear_vel, angular_vel, lateral_vel, steer_angle / 10.0);
  }

  void SetLightCommand(LightMode f_mode, uint8_t f_value, LightMode r_mode, uint8_t r_value) override
  {
    AgilexBase<ProtocolV2Parser>::SendLightCommand(f_mode, f_value, r_mode, r_value);
  }
