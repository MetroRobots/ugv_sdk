...
bool DecodeCanFrameV1(const struct can_frame* rx_frame, AgxMessage* msg)
{
  ...
  if (!CalcCanFrameChecksumV1(rx_frame->can_id, (uint8_t*)rx_frame->data, rx_frame->can_dlc))
  {
  ...
  case CAN_MSG_SYSTEM_STATE_ID:
  {
    ...
    msg->body.system_state_msg.battery_voltage =
        ((((uint16_t)rx_frame->data[2]) << 8) | (uint16_t)rx_frame->data[3]) / 10.0f;
    msg->body.system_state_msg.error_code = ((((uint16_t)rx_frame->data[4]) << 8) | (uint16_t)rx_frame->data[5]);
    break;
  }
  case CAN_MSG_MOTION_STATE_ID:
  {
    ...
    msg->body.motion_state_msg.linear_velocity =
        ((int16_t)((((uint16_t)rx_frame->data[0]) << 8) | (uint16_t)rx_frame->data[1])) / 1000.0f;
    msg->body.motion_state_msg.angular_velocity =
        ((int16_t)((((uint16_t)rx_frame->data[2]) << 8) | (uint16_t)rx_frame->data[3])) / 1000.0f;
    msg->body.motion_state_msg.lateral_velocity =
        ((int16_t)((((uint16_t)rx_frame->data[4]) << 8) | (uint16_t)rx_frame->data[5])) / 1000.0f;
    break;
  }
  ...
  case CAN_MSG_ACTUATOR4_STATE_ID:
  {
    ...
    msg->body.v1_actuator_state_msg.motor_id = rx_frame->can_id - CAN_MSG_ACTUATOR1_STATE_ID;
    msg->body.v1_actuator_state_msg.current =
        ((((uint16_t)rx_frame->data[0]) << 8) | (uint16_t)rx_frame->data[1]) / 10.0f;
    msg->body.v1_actuator_state_msg.rpm =
        (int16_t)(((((uint16_t)rx_frame->data[2]) << 8) | (uint16_t)rx_frame->data[3]));
...
bool EncodeCanFrameV1(const AgxMessage* msg, struct can_frame* tx_frame)
{
  ...
  case AgxMsgMotionCommandV1:
  {
    ...
    frame.linear_percentage = (int8_t)(msg->body.v1_motion_command_msg.linear);
    frame.angular_percentage = (int8_t)(msg->body.v1_motion_command_msg.angular);
    frame.lateral_percentage = (int8_t)(msg->body.v1_motion_command_msg.lateral);
    frame.reserved0 = 0;
    frame.count = count++;
    memcpy(tx_frame->data, (uint8_t*)(&frame), tx_frame->can_dlc);
    tx_frame->data[7] = CalcCanFrameChecksumV1(tx_frame->can_id, tx_frame->data, tx_frame->can_dlc);
    break;
  }
  case AgxMsgValueSetCommandV1:
  {
    ...
    frame.set_neutral = msg->body.v1_value_set_command_msg.set_neutral ? 0xaa : 0x00;
