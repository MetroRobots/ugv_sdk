...
void ScoutRobot::EnableCommandedMode()
{
  robot_->EnableCommandedMode();
}

std::string ScoutRobot::sendRequest()
{
  return robot_->sendRequest();
}

void ScoutRobot::Connect(std::string can_name)
{
...
void ScoutRobot::ResetRobotState()
{
...
void ScoutRobot::SetLightCommand(LightMode f_mode, uint8_t f_value, LightMode r_mode, uint8_t r_value)
{
  auto scout = dynamic_cast<ScoutInterface*>(robot_);
...
void ScoutMiniOmniRobot::SetMotionCommand(double linear_vel, double angular_vel, double lateral_velocity)
{
