...
{
AsyncCAN::AsyncCAN(std::string can_port) : AsyncPortBase(can_port), socketcan_stream_(io_context_)
{
}
...
bool AsyncCAN::SetupPort()
{
  ...
  io_context_.post(std::bind(&AsyncCAN::ReadFromPort, this, std::ref(rcv_frame_), std::ref(socketcan_stream_)));
#else
  asio::post(io_context_, std::bind(&AsyncCAN::ReadFromPort, this, std::ref(rcv_frame_), std::ref(socketcan_stream_)));
#endif
...
void AsyncCAN::ReadFromPort(struct can_frame& rec_frame, asio::posix::basic_stream_descriptor<>& stream)
{
  ...
  stream.async_read_some(asio::buffer(&rec_frame, sizeof(rec_frame)),
...
{
  socketcan_stream_.async_write_some(asio::buffer(&frame, sizeof(frame)),
