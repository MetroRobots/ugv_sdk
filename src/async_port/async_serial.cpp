...
AsyncSerial::AsyncSerial(std::string port_name, uint32_t baud_rate)
  : AsyncPortBase(port_name), baud_rate_(baud_rate), serial_port_(io_context_)
{
}
...
bool AsyncSerial::SetupPort()
{
  ...
  try
  {
    ...
    serial_port_.set_option(SPB::flow_control((hwflow_) ? SPB::flow_control::hardware : SPB::flow_control::none));
...
void AsyncSerial::ReadFromPort()
{
  ...
  serial_port_.async_read_some(asio::buffer(rx_buf_), [sthis](asio::error_code error, size_t bytes_transferred) {
    if (error)
    {
      sthis->StopService();
      return;
    ...
    {
      sthis->rcv_cb_(sthis->rx_buf_.data(), sthis->rx_buf_.size(), bytes_transferred);
    }
    else
    {
      sthis->DefaultReceiveCallback(sthis->rx_buf_.data(), sthis->rx_buf_.size(), bytes_transferred);
    }
    sthis->ReadFromPort();
  });
}
...
void AsyncSerial::WriteToPort(bool check_if_busy)
{
  ...
  serial_port_.async_write_some(asio::buffer(tx_buf_, len), [sthis](asio::error_code error, size_t bytes_transferred) {
    if (error)
    {
      sthis->StopService();
      return;
