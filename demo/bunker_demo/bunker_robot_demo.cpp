...
int main(int argc, char** argv)
{
  ...
  if (argc == 3)
  {
    ...
    device_name = {argv[2]};
    std::cout << "Use protocol " << protocol_version << " on interface " << device_name << std::endl;
  }
  else
  {
    std::cout << "Usage: app_bunker_demo <protocol-version> <interface>" << std::endl
  ...
  {
    bunker = std::unique_ptr<BunkerRobot>(new BunkerRobot(ProtocolVersion::AGX_V1));
  }
  else if (protocol_version == "v2")
  {
    bunker = std::unique_ptr<BunkerRobot>(new BunkerRobot(ProtocolVersion::AGX_V2));
  }
  else
  {
