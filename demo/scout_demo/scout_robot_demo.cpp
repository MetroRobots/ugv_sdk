...
int main(int argc, char** argv)
{
  ...
  {
    device_name = {argv[1]};
    std::cout << "Selected interface " << device_name << ", robot type: scout" << std::endl;
  }
  else if (argc == 3)
  {
    robot_subtype = {argv[1]};
    device_name = {argv[2]};
    std::cout << "Selected interface " << device_name << ", robot type: " << robot_subtype << std::endl;
  }
  else
  {
    std::cout << "Usage: demo_scout_robot [<robot-subtype>] <interface>" << std::endl
  ...
  {
    std::cout << "Unknown robot subtype. Supported subtypes: \"scout\" or \"mini\"" << std::endl;
  }
  ...
  if (detector.Connect(device_name))
  {
    ...
    if (proto == ProtocolVersion::AGX_V1)
    {
      ...
      scout = std::unique_ptr<ScoutRobot>(new ScoutRobot(ProtocolVersion::AGX_V1, is_scout_mini));
    }
    else if (proto == ProtocolVersion::AGX_V2)
    {
      std::cout << "Detected protocol: AGX_V2" << std::endl;
      scout = std::unique_ptr<ScoutRobot>(new ScoutRobot(ProtocolVersion::AGX_V2, is_scout_mini));
    }
    else
    {
  ...
  while (true)
  {
    ...
    std::cout << "control mode: " << static_cast<int>(state.system_state.control_mode)
              ...
              << " , error code: " << std::hex << state.system_state.error_code << std::dec
    ...
    std::cout << "core state age (ms): "
              << std::chrono::duration_cast<std::chrono::milliseconds>(AgxMsgRefClock::now() - state.time_stamp).count()
              << std::endl;
    ...
    if (scout->GetParserProtocolVersion() == ProtocolVersion::AGX_V1)
    {
      for (int i = 0; i < 4; ++i)
      {
        printf("motor %d: current %f, rpm %d, driver temp %f, motor temp %f\n", actuator.actuator_state[i].motor_id,
               actuator.actuator_state[i].current, actuator.actuator_state[i].rpm,
               actuator.actuator_state[i].driver_temp, actuator.actuator_state[i].motor_temp);
      }
      std::cout
          << "actuator state age (ms): "
          << std::chrono::duration_cast<std::chrono::milliseconds>(AgxMsgRefClock::now() - actuator.time_stamp).count()
          << std::endl;
    }
    else
    {
      for (int i = 0; i < 4; ++i)
      {
        printf("motor %d: current %f, rpm %d, driver temp %f, motor temp %f\n", actuator.actuator_hs_state[i].motor_id,
               actuator.actuator_hs_state[i].current, actuator.actuator_hs_state[i].rpm,
               actuator.actuator_ls_state[i].driver_temp, actuator.actuator_ls_state[i].motor_temp);
      }
      std::cout
          << "actuator state age (ms): "
          << std::chrono::duration_cast<std::chrono::milliseconds>(AgxMsgRefClock::now() - actuator.time_stamp).count()
